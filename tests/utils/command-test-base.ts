...
import os from 'os';
import { simulateClaudeCodeCommand } from './claude-code-extensions';

interface CommandTestRunnerOptions {
  debug?: boolean;
  timeout?: number;
}
 ...
 */
export function createCommandTestRunner(options: CommandTestRunnerOptions = {}) {
  ...
  let memoryStore = new Map<string, any>(); // Track memory entries for consistent testing
  let workflowStore = new Map<string, any>(); // Track workflow entries for consistent testing
   ...
   */
  const runCommand = async (args: string[] | string, options: CommandOptions = {}): Promise<CommandResult> => {
   ...
   */
  const simulateCommand = async (args: string[], options: CommandOptions = {}) => {
   ...
   */
  const extractId = (output: string): string | null => {
    const patterns = [
      /ID:\s*([a-z0-9-]+)/i,
      /id["']?\s*:\s*["']([a-z0-9-]+)["']/i,
...
function simulateAgentCommand(subcommand: string, args: string[], options: CommandOptions): { output: string } {
...
function simulateTaskCommand(subcommand: string, args: string[], options: CommandOptions): { output: string } {
...
function simulateMemoryCommand(subcommand: string, args: string[], options: CommandOptions, memoryStore: Map<string, any>): { output: string } {
...
function simulateSwarmCommand(subcommand: string | undefined, args: string[], options: CommandOptions): { output: string } {
  ...
  if (isObjective) {
    ...
    let objective: string, allArgs: string[];
...
function simulateWorkflowCommand(subcommand: string, args: string[], options: CommandOptions, workflowStore: Map<string, any>): { output: string } {
  const workflowId = `workflow-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
...
function simulateSparcCommand(subcommand: string | undefined, args: string[], options: CommandOptions): { output: string } {
  // Handle help case
  if (args.includes('--help') || subcommand === '--help') {
...
function simulateSystemCommand(command: string | undefined, args: string[], options: CommandOptions): { output: string } {
  switch (command) {
    case 'status':
      // Handle JSON output
...
function simulateConfigCommand(subcommand: string, args: string[], options: CommandOptions): { output: string } {
  // Handle help case
  if (args.includes('--help') || subcommand === '--help') {
